...
import com.example.tfg.clases.Datos_S
import com.example.tfg.clases.Estado_Semanal
import com.example.tfg.clases.Grafica_S
import com.example.tfg.modelos.Datos_Grafica_S
...
@Composable
fun PantallaGraficaSemanal(navController: NavHostController, datos_dia : Datos_Grafica_S = viewModel()) {
    ...
    Scaffold(
        ...
        Box(
            ...
            Column(modifier= Modifier.verticalScroll(rememberScrollState()), horizontalAlignment = Alignment.CenterHorizontally) {
                Encabezado_Semanal(navController)
                RecogerDatosS(datos_dia)
...
@Composable
private fun RecogerDatosS(datosDia: Datos_Grafica_S) {
    ...
    when(val result = datosDia.state.value){
        is Estado_Semanal.Cargando -> {
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center){
                Column(horizontalAlignment = Alignment.CenterHorizontally){
                    Text(text = "Cargando las datos semanales")
                    Spacer(modifier = Modifier.padding(16.dp))
                    LinearProgressIndicator()
                }
            }
        }
        is Estado_Semanal.Exito -> {
            Barras_S(result.datos)
        }
        is Estado_Semanal.Fallo -> {
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center){
                Text(text = result.mensaje)
            }
        }
        is Estado_Semanal.Vacio -> {
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center){
...
@Composable
private fun Encabezado_Semanal(navController: NavHostController) {
    ...
    datos.mapIndexed { index, datos ->
        barras.add(
            BarChartData.Bar(
                ...
                color = Color_Consumo_S(datos.consumo.toString().toFloat())
            )
        )
    }
...
private fun Contenido_GS() {
    ...
    Card(
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            Text(
                text = "Este es el gráfico de su consumo durante el día actual. En caso de superar los 25.63 kw/h, sus barras se mostrarán en color rojo, en caso de ser el consumo inferior a este y hasta los 12.401 kw/h, estas se mostrarán de color amarillo, y si es inferior, se mostrará verde.",
                color = Color.Black
            )
        }
    }
